...
def load_data(filepath: str) -> pd.DataFrame:
    ...
    required_cols = [
        'crsp_date', 'ret', 'market_cap', 'exchcd', 'permno', 'FF_year',
        'goodwill_to_equity_lagged', 'gdwl', 'ceq', 'sich', 'siccd'
    ]
    
    try:
        df = pd.read_csv(filepath, parse_dates=['crsp_date', 'compustat_date'])
        ...
        numeric_cols = ['ret', 'market_cap', 'exchcd', 'gdwl', 'ceq']
...
def compute_industry_adjusted_gte(df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute industry-adjusted GTE as of June 30 each year.
    
    Args:
        df (pd.DataFrame): Input DataFrame with required columns.
    
    Returns:
        pd.DataFrame: DataFrame with industry-adjusted GTE column.
    """
    logger.info("Computing industry-adjusted GTE...")

    # Map SIC codes to FF 48 industries
    df['sic'] = df['sich'].fillna(df['siccd'])  # Use sich, fallback to siccd
    df['ff48_industry'] = df['sic'].apply(map_sic_to_ff48)

    # Calculate GTE (recompute for clarity, even though goodwill_to_equity_lagged exists)
    df['gte'] = df['gdwl'] / df['ceq']
    df['gte'] = df['gte'].replace([np.inf, -np.inf], np.nan)

    # Winsorize GTE at 1% and 99% (as in File 2)
    lower, upper = df['gte'].quantile([0.01, 0.99])
    df['gte'] = df['gte'].clip(lower, upper)

    # For each FF_year, use the latest GTE per firm
    df_latest = df.sort_values(['permno', 'compustat_date']).groupby(['permno', 'FF_year']).tail(1)

    # Compute industry average GTE per FF_year and industry
    industry_avg = (df_latest.groupby(['FF_year', 'ff48_industry'])
    ...
    df_latest = df_latest.merge(industry_avg[['industry_avg_gte']], 
                               on=['FF_year', 'ff48_industry'], how='left')

    # Compute industry-adjusted GTE
    df_latest['gte_industry_adj'] = df_latest['gte'] - df_latest['industry_avg_gte']

    # Keep only firms in industries with at least 4 firms
    df_latest = df_latest[df_latest['industry_avg_gte'].notna()]

    # Merge back to the full dataset
    df = df.merge(df_latest[['permno', 'FF_year', 'gte_industry_adj', 'ff48_industry']], 
                  on=['permno', 'FF_year'], how='left')

    logger.info(f"Industry-adjusted GTE computed. Rows with adjustment: {df['gte_industry_adj'].notna().sum()}")
    return df
...
def main(filepath: str, ff48_filepath: str) -> None:
    """
    Main function to construct industry-adjusted GTE factors and save results.
    
    Args:
        filepath (str): Path to the processed data CSV file.
        ff48_filepath (str): Path to the Fama-French 48 industry classification Excel file.
    """
    # Load the Fama-French 48 industry mapping
    global FF48_MAPPING
    ...
    # Assign Small/Big based on sample median using june_me_ff_style
    ...
    # Metric to process (industry-adjusted GTE)
    metric = 'gte_industry_adj'
    
    logger.info(f"Processing {metric}...")
    count_firms_per_year(df_full, metric)
    # Log missing metric values in the full dataset
    missing_metric = df_full[metric].isna().sum()
    logger.info(f"Firms with missing {metric} in full dataset: {missing_metric} ({missing_metric / len(df_full) * 100:.2f}%)")
    
    # Log missing metric values in the filtered dataset
    missing_metric_filtered = df[metric].isna().sum()
    logger.info(f"Firms with missing {metric} in filtered dataset: {missing_metric_filtered} ({missing_metric_filtered / len(df) * 100:.2f}%)")
    ...
    for year in range(2004, 2024):
        ...
        # Log avg metric per decile for diagnostics
